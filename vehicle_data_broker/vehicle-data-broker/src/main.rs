...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parser = Command::new("Vehicle Data Broker")
        .version(version)
        .about(&*about)
        .arg(
            Arg::new("address")
        ...
        .arg(
            Arg::new("metadata")
                ...
                .takes_value(true)
                .value_name("FILE")
                .env("VEHICLE_DATA_BROKER_METADATA_FILE")
                .required(false),
        )
        .arg(
    ...
    info!("Starting Vehicle Data Broker {}", version);
    let addr = format!(
        "{}:{}",
