...
sed -i -E "s/^version = \"${VERSION_REGEX}\"$/version = \"${VERSION}\"/" \
	"$DATA_BROKER_ROOT/vehicle-data-api/Cargo.toml" \
	"$DATA_BROKER_ROOT/vehicle-data-broker/Cargo.toml" \
	"$DATA_BROKER_ROOT/vehicle-data-cli/Cargo.toml" \
	"$DATA_BROKER_ROOT/vehicle-data-examples/Cargo.toml"
