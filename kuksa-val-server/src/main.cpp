...
int main(int argc, const char *argv[]) {
  ...
  desc.add_options()
    ...
    ("config-file,c", program_options::value<boost::filesystem::path>()->default_value(boost::filesystem::path{"config.ini"}),
      ...
      "vss = vss_rel_3.0.json\n"
      "cert-path = . \n"
      "log-level = ALL\n")
    ("vss", program_options::value<boost::filesystem::path>()->required(), "[mandatory] Path to VSS data file describing VSS data tree structure which `kuksa-val-server` shall handle. Sample 'vss_rel_3.0.json' file can be found under [data](./data/vss-core/vss_rel_3.0.json)")
